...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      [[ $target_set -eq 0 ]] || { echo "Unexpected argument: $1" >&2; exit 2; }
      TARGET_PATH="$1"; target_set=1; shift
      ;;
  esac
done
...
resolve_ai_entries() {
  local selection="$1" raw item allowed matched
  local output=""
  if [[ -z "$selection" ]]; then
    if [[ $YES -eq 1 ]]; then return; fi
    while true; do
      printf "Select AI entry files [%s or none/all]: " "$(IFS=', '; echo "${ENTRY_KEYS[*]}")" >&2
...
[[ ! -e "$DEPLOY_PATH" ]] || reject_symlink_components "$DEPLOY_PATH"

existing=0
if [[ -d "$DEPLOY_PATH" ]]; then
  existing=1
  if [[ $REUSE_EXISTING -eq 1 || ( $YES -eq 1 && $REPLACE_EXISTING -eq 0 ) ]]; then echo "Reusing existing deployment at $DEPLOY_PATH"; exit 0; fi
  if [[ $REPLACE_EXISTING -eq 0 ]]; then
    choice="$(ask_choice "Existing deployment found. Choose action" reuse replace custom cancel)"
    case "$choice" in
      reuse) echo "Reusing existing deployment at $DEPLOY_PATH"; exit 0 ;;
      replace) REPLACE_EXISTING=1 ;;
      custom)
        printf "Enter new folder name: " >&2; IFS= read -r DEPLOY_FOLDER_NAME
        validate_folder_name "$DEPLOY_FOLDER_NAME"
        DEPLOY_PATH="$REPO_PATH/$DEPLOY_FOLDER_NAME"
        [[ ! -e "$DEPLOY_PATH" ]] || { echo "Custom deployment path already exists: $DEPLOY_PATH" >&2; exit 1; }
        existing=0
        ;;
      cancel) echo "Cancelled."; exit 1 ;;
    esac
  fi
  if [[ $REPLACE_EXISTING -eq 1 && ( ! -f "$DEPLOY_PATH/PROMPT.md" || ! -d "$DEPLOY_PATH/templates" ) ]]; then
    echo "Refusing to replace an unmarked deployment. PROMPT.md and templates/ are required markers." >&2; exit 1
  fi
fi
...
BACKUP_ROOT=""
[[ $existing -eq 0 ]] || BACKUP_ROOT="$DEPLOY_PATH/.backup/install-$OPERATION_ID"
BACKUP_BASE="${BACKUP_ROOT:-$STAGE_ROOT/rollback}"
APPLIED_PATHS=(); APPLIED_BACKUPS=(); APPLIED_HAD=(); APPLIED_DIR=()
created_deployment=$((1-existing))
...
rollback() {
  ...
  [[ $created_deployment -eq 0 || ! -d "$DEPLOY_PATH" ]] || rm -rf -- "$DEPLOY_PATH"
}
...
for f in "${MANAGED_FILES[@]}"; do
  dest="$DEPLOY_PATH/$f"; backup="$BACKUP_BASE/deployment/$f"
  backup_and_track "$dest" "$backup" 0
  cp -- "$STAGE_DEPLOY/$f" "$dest"
  chmod u+rw "$dest"
done
templates_dest="$DEPLOY_PATH/templates"; templates_backup="$BACKUP_BASE/deployment/templates"
backup_and_track "$templates_dest" "$templates_backup" 1
[[ ! -e "$templates_dest" ]] || { reject_symlink_components "$templates_dest"; rm -rf -- "$templates_dest"; }
...
[[ -z "$BACKUP_ROOT" || ! -d "$BACKUP_ROOT" ]] || chmod -R go-rwx "$BACKUP_ROOT" 2>/dev/null || true
trap - ERR INT TERM
cleanup_stage
